...
int main(int argc, char **argv) {
    ...
    syslog(LOG_NOTICE, "Program started by User with ID = %d", getuid());
    
    // Zapisanie bieżącego kodu błędu do dziennika
    syslog(LOG_NOTICE, "Wynik operacji = %m");
    
    // Zapisanie informacyjnego komunikatu do dziennika
    syslog(LOG_INFO, "Wysyłam log informacyjny!");
    
    // Zapisanie komunikatu o błędzie do dziennika
    syslog(LOG_ERR, "Wysyłam komunikat o błędzie!");
